...
`asr_final` 不一定来自独立 ASR。若 `AudioLLMProvider.transcribesInput` 为
`true`，Core 会把同一 audio-turn 响应中的权威输入转写提交为 `asr_final`；这种
配置可以省略 `providers.asr`。未声明该能力时，仍由 caption ASR 产生用户字幕。
