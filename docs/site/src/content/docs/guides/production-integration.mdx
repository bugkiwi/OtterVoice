...
可运行的 [Audio LLM-only 示例](https://github.com/bugkiwi/OtterVoice/tree/main/examples/web-audio-llm-only) 的客户端不包含模型、prompt、voice 或生成参数；服务端通过 `createOpenRouterGateway()` 重建请求、校验 `Origin` 并限制请求体/历史/文本。请把强制 `authorize` 钩子接入自己的用户与会话鉴权；示例不是完整账号服务。
...
只重试 `retryable` 错误，在网关使用带随机抖动的指数退避并遵守供应商 Retry Header。鉴权、额度、音频解码、参数校验失败不能在请求不变时盲目重试。客户端 `audioLlmRetry`、滚动 ASR、`after_audio` 和 pipeline 选择都可能增加请求/成本；服务端预算与幂等控制不能依赖这些客户端值。
