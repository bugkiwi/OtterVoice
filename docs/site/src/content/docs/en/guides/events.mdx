...
`asr_final` does not have to come from a separate ASR. When
`AudioLLMProvider.transcribesInput` is `true`, Core commits the authoritative
input transcript from the same audio-turn response as `asr_final`, and
`providers.asr` may be omitted. Otherwise caption ASR owns user subtitles.
