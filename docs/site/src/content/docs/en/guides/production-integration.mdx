...
The composite voice factory implements `AudioLLMProvider` with
`transcribesInput: true`. With `createOtterVoiceSession`, clients may omit
caption `asr`; the gateway must still authorize, rate-limit, and meter the
`asr_llm_tts` profile independently.
