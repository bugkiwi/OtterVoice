...
`start()` and `stop()` are safe to overlap across React Native lifecycle
changes. If native microphone, sound, or playlist setup completes after
`stop()`, the runtime immediately stops or destroys that stale object instead
of resuming work in the background. Temporary files created by `writeAudioFile`
and `writePcmChunk` are best-effort removed through `deleteAudioFile` on normal,
