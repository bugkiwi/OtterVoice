...
Core is cross-runtime orchestration, so prompt, temperature, max-token, and
voice controls remain available for trusted server sessions. They are not a
browser security boundary. Browser/app integrations should use server-managed
providers and let their application gateway override every business and cost
policy field.
